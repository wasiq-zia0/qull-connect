"""Stripe billing for the bill-negotiator connector.
...
Flow:
  ...
  3. User confirms documented savings -> confirm_savings() charges 35% of the
     documented savings off-session via PaymentIntent.
...
The stripe CLI is NEVER invoked during tests (see app billing setup path).
"""
import json
import os
import subprocess
...
def _stripe(*args: str, idempotency_key: str | None = None) -> dict:
    ...
    NOTE: the local skill CLI has no idempotency-key flag, so idempotency_key
    is honored by the REST transport only and silently ignored on the CLI
    fallback (dev machines only; the app-level already-charged guard still
...
def setup_customer(name: str, email: str = "", idempotency_key: str | None = None) -> dict:
    """Create a Stripe customer for the user. Returns {'customer_id': ...} or {'error': ...}."""
...
def charge_fee(customer_id: str, amount_cents: int, description: str,
    ...
    """Off-session charge of the 35% fee against the saved card."""
...
def fee_cents(documented_savings: float) -> int:
    """35% of documented savings, in cents."""
    return int(round(documented_savings * FEE_RATE * 100))
