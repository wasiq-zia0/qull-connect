"""Stripe billing for the eu261-flight-comp connector.

Flow:
  1. Claim created -> setup_customer() creates a Stripe customer, stored on the claim.
  2. Passenger saves a card -> create_card_setup() returns a SetupIntent client_secret;
     the frontend/connector collects the card against it (card stored for off-session use).
  3. Airline payout confirmed by the user -> confirm_payout() charges the 30%
...
The CLI is NEVER invoked during automated testing.
"""
import json
import os
import subprocess
...
def setup_customer(name: str, email: str = "") -> dict:
    """Create a Stripe customer for the passenger. Returns {'customer_id': ...} or {'error': ...}."""
...
def create_card_setup(customer_id: str, idempotency_key: str | None = None) -> dict:
    """Create a SetupIntent so the passenger can save a card for the later contingency charge."""
...
def charge_fee(customer_id: str, amount_cents: int, description: str,
    ...
    """Off-session charge of the 30% contingency fee against the saved card."""
...
def fee_cents(amount_eur: float) -> int:
    """30% contingency fee on the confirmed payout, in EUR cents."""
    return int(round(amount_eur * FEE_RATE * 100))
