"""Stripe billing for the deposit-recovery connector.

Flow:
  1. Case created -> setup_customer() creates a Stripe customer, stored on the case.
  2. Tenant saves a card -> create_card_setup() returns a SetupIntent client_secret;
     the frontend/connector collects the card against it (card stored for off-session use).
  3. Recovery confirmed -> confirm_recovery() charges the 25% contingency fee
...
def setup_customer(name: str, email: str = "") -> dict:
    """Create a Stripe customer for the tenant. Returns {'customer_id': ...} or {'error': ...}."""
...
def create_card_setup(customer_id: str, idempotency_key: str | None = None) -> dict:
    """Create a SetupIntent so the tenant can save a card for the later contingency charge."""
