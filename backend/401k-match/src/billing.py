"""401k-match pricing and shared Stripe billing.
...
FEE_DISCLOSURE = 'A one-time $99 fee for this annual plan pack. There is no automatic renewal. Charged only after you explicitly confirm.'
...
def charge_fee(customer_id: str, idempotency_key: str | None = None, *,
                setup_intent_id: str | None = None, checkout_session_id: str | None = None,
                consent: bool = False) -> dict:
    result = _client.charge_fee(customer_id, ANNUAL_FEE_CENTS, 'MatchMax annual plan pack (one-time fee, no automatic renewal)', idempotency_key,
